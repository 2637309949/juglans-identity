...
module.exports = function ({ router, setToken, getToken, fakeTokens, fakeUrls, model }) {
  router.use(async function (ctx, next) {
    try {
      ...
      const { isFakeTokens, isFakeUrls } = await utils.fakeVerify(ctx.path, { fakeTokens, fakeUrls, accessToken: token.accessToken })
      if (isFakeUrls) {
        ...
        token = await model.findToken(token.accessToken)
        if (!token) {
          ctx.status = 401
          ctx.body = {
